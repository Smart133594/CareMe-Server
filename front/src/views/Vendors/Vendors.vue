            ...
            <v-row>
              ...
              >
                <v-img
                  ...
                  class="my-2"
                  :src="item.image ? item.image : '/static/img/gallery.png'"
                  aspect-ratio="1"
                  contain
                ></v-img>
              </v-col>
...
export default {
  components: { UserSidebar, VendorArea },
  data: function () {
    return {
      all: true,
      loading: false,
  ...
  methods: {
    ...
    edit(item) {
      ...
      for (let index = 0; index < 4 - item.images.length; index++) {
        this.images.push({ id: -1, image: null });
      }
    ...
    saveVendorRole() {
      var new_array = this.providers.map(function (value) {
        return { id: value.id, roles:value.roles};
      });
      var model = {
          roles:new_array
      }
