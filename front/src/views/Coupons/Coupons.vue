            ...
            <template v-slot:body="{ items }">
              <tbody>
                <tr v-for="(item, index) in items" :key="`service${item.id}`">
                  ...
                  <td>
                    <v-btn  v-if="item.available == 1" class="ma-2" color="primary" @click="edit(item)">{{
                      $t("message.edit")
                    }}</v-btn>
                    <v-btn
                      class="ma-2"
          ...
          <v-form v-model="valid">
            ...
            <v-row>
              <v-menu
                ...
                :nudge-right="40"
                min-width="290px"
                :return-value.sync="model.expire"
              >
                <template v-slot:activator="{ on }">
                  <v-text-field
                    v-on="on"
                    :label="$t('message.expireDate')"
                    v-model="model.expire"
                    prepend-icon="event"
                    readonly
                    :rules="requireRule"
                    required
                ...
                <v-date-picker v-model="model.expire" no-title scrollable>
                  ...
                  <v-btn
                    color="warning"
                    @click="$refs.expire_date.save(model.expire)"
                    >OK</v-btn
                  >
                </v-date-picker>
              </v-menu>
...
export default {
  ...
  methods: {
    ...
    deleteCoupon(item) {
      this.loading = true;
      api
        .post("deleteCoupon", JSON.stringify({ id: item.id }), {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${this.getUser.token}`,
          },
